...
# EXERCISE 1: Python's heapq is a MIN-heap, but we want the LARGEST prettiness.
# How can we make a max-heap using negation?
# TODO: Replace the ? with the correct value to push the PEN with HIGHEST prettiness first
color_heaps = [[] for _ in range(M + 1)]

for i in range(1, N + 1):
    C[i], P[i] = map(int, input().split())
    heapq.heappush(color_heaps[C[i]], (?, i))  # What should go in the ??
...
def refresh_color(c):
    ...
    # EXERCISE 2: Implement lazy deletion.
    # Some heap entries are "stale" (the pen moved to a different color or changed prettiness).
    # Keep popping stale entries until you find a valid one where:
    # - C[idx] == c (pen is still in this color)
    # - P[idx] == -neg_p (pen's prettiness matches what's in the heap)
    # TODO: Fill in the while loop and if conditions below
    while color_heaps[c]:
        neg_p, idx = color_heaps[c][0]
        # FILL IN: What condition checks if this entry is VALID (not stale)?
        if ?:
            break
...
def get_answer():
    # EXERCISE 3: Clean up the global_bests heap.
    # We may have added duplicate/stale entries for the same color.
    # Pop until best_per_color[c] matches what's at the top of the heap.
    # TODO: Fill in the while and if conditions
    while global_bests:
        p, c = global_bests[0]
        # FILL IN: What condition means this entry is current (not stale)?
        if ?:
            break
